...
def main_with_streamlit(folder_path, config_path):
    ...
    for i, x in enumerate(folder_name_list):
        ...
        st.progress(i / len(folder_name_list))
        ...
        try:
            ...
            # 保存数据
            save_path = os.path.join(folder_path.replace('原始网银流水', '整理后网银流水_auto'))
    ...
    log_file_path = os.path.join(folder_path.replace('原始网银流水', '整理后网银流水_auto'), '合并日志.xlsx')
...
if __name__ == "__main__":
    ...
    # 用户上传配置文件
    config_file = st.file_uploader("上传配置映射表:", type=['xlsx'])
